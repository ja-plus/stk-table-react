# Introduce
`stk-table-react` (stk: sticky) is a **high-performance** virtual list based on `sticky`, `react`, and `DOM`. It is suitable for dynamic data display scenarios.


## Why stk-table-react was Created
...
3. **DOM-based.** Being DOM-based facilitates seamless integration with React component libraries. Moreover, I have great confidence in DOM performance. After considering various costs, I abandoned the canvas implementation.
...
5. **Declarative Custom Cells.** Borrowed from `ant-design` table's custom cell approach, passing a React component via `customCell`, which is very elegant to use.
...
Some designs and APIs of the component refer to excellent projects like `vxe-table`, `ant-design`, and `navie-ui`. I would like to express my gratitude here.
