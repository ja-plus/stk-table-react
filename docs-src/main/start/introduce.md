# 介绍
`stk-table-react`（stk: sticky） 是一个基于 `sticky` 、`react`、`DOM` 的**高性能**虚拟列表。适用于动态数据展示场景。


## 为什么要做stk-table-react
...
3. **基于DOM。** 基于 DOM 有利于 react 组件库的无缝接入。而且，我十分相信dom的性能。综合各种成本考虑，舍弃了canvas的实现。
...
5. **声明式自定义单元格。** 借鉴了 `ant-design` 表格的自定义单元格的方式，通过 `customCell` 传入 React 组件，使用十分优雅。
...
## 站在巨人的肩上
组件的一些设计和api参考了 `vxe-table` `ant-design` `navie-ui` 等NB的项目，在此表示感谢。
