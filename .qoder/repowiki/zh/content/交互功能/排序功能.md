...
- [docs-demo/advanced/custom-sort/InsertSort.tsx](file://docs-demo/advanced/custom-sort/InsertSort.tsx)
- [docs-src/main/table/basic/sort.md](file://docs-src/main/table/basic/sort.md)
</cite>

## 更新摘要
**变更内容**   
- 更新了自定义排序算法实现，改进了 InsertSort.tsx 中的排序逻辑
- 优化了排序性能与稳定性，增强了复杂数据场景下的处理能力
- 完善了空值处理策略和边界条件检查
...
**更新** 本次更新重点改进了自定义排序算法的实现，特别是在 InsertSort.tsx 中优化了排序逻辑，提升了复杂数据场景下的性能和稳定性。
...
- **更新** 改进的插入排序算法提供了更稳定的排序结果和更好的性能表现，特别是在处理大量数据和复杂比较逻辑时。
...
- **更新** 改进的插入排序算法在处理复杂业务逻辑时提供了更好的稳定性和性能，特别是对于需要多次比较和交换的场景。
- 参考示例：
  - 高级自定义排序综合案例：[docs-demo/advanced/custom-sort/CustomSort/index.tsx](file://docs-demo/advanced/custom-sort/CustomSort/index.tsx)
  - 优化的插入排序实现：[docs-demo/advanced/custom-sort/InsertSort.tsx](file://docs-demo/advanced/custom-sort/InsertSort.tsx)
...
- **更新** 改进的插入排序算法在处理中等规模数据时具有更好的性能表现，减少了不必要的内存分配和比较操作。
...
## 结论
StkTable 的排序功能提供了从简单到复杂的完整能力矩阵：单列与多列排序开箱即用，字段级与全局自定义比较器满足多样化业务需求，空值处理策略灵活可控，远程排序与事件驱动设计使前后端协作顺畅。**更新** 本次改进的插入排序算法进一步提升了复杂场景下的性能和稳定性，使得开发者能够构建出更高效、更可靠的排序解决方案。通过合理的状态管理与性能优化，开发者可以构建出高效、稳定且用户体验优秀的排序方案。
